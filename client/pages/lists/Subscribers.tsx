    ...
    return (
        <div className="animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                <div>
                    <h1 className="text-xl font-bold text-slate-800 dark:text-white tracking-tight">Audience & Campaigns</h1>
                ...
                <div className="flex gap-3">
                    ...
                    >
                        <Send size={14} className="mr-2" /> Create Campaign
                    </button>
                </div>
            </div>

            {/* Navigation Tabs */}
            <div className="flex space-x-1 bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-xl mb-3 w-fit border border-slate-200 dark:border-slate-700/60">
                <button
                    onClick={() => setActiveTab('subscribers')}
                    className={`px-3 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center ${activeTab === 'subscribers' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
                >
                    <Users size={16} className="mr-2" /> Subscribers
                </button>
                <button
                    onClick={() => setActiveTab('campaigns')}
                    className={`px-3 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center ${activeTab === 'campaigns' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
                >
                    ...
                    <div className="bg-white dark:bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60 overflow-hidden">
                        {loading ? (
                            ...
                            <>
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700/60">
                                        <thead className="bg-slate-50/50 dark:bg-slate-900/50">
                                            <tr>
                                                <th className="px-3 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Subscriber</th>
                                                <th className="px-3 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Plan</th>
                                                <th className="px-3 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Source</th>
                                                <th className="px-3 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Joined</th>
                                                <th className="px-3 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                                                <th className="px-3 py-4 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Actions</th>
                                            </tr>
                                        </thead>
                                            ...
                                            {filteredSubscribers.length > 0 ? (
                                                filteredSubscribers.map((sub) => (
                                                    <tr key={sub.id} className="group hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors duration-200">
                                                        ...
                                                        <td className="px-3 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400 font-medium">
                                                            {sub.source || 'Website'}
                                                        </td>
                                                        <td className="px-3 py-4 whitespace-nowrap">
                                ...
                                {/* Pagination Visual */}
                                <div className="px-3 py-4 border-t border-slate-200 dark:border-slate-700/60 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/20">
                        ...
                        {newslettersLoading ? (
                            ...
                            filteredNewsletters.map((news) => (
                                <div
                                    ...
                                    className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 hover:border-blue-500/50 hover:shadow-md transition-all cursor-pointer group flex flex-col md:flex-row gap-3 items-start md:items-center"
                                >
                                    {/* Icon Status Box */}
                                    <div className={`shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center ${news.status === 'Sent' ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' :
                                        ...
                                        {news.status === 'Sent' ? <CheckCircle size={16} /> : news.status === 'Scheduled' ? <Clock size={16} /> : <Mail size={16} />}
                                    </div>
                                    ...
                                    {news.status === 'Sent' && (
                                        <div className="flex space-x-3 border-l border-slate-100 dark:border-slate-700 pl-3 py-1">
                                            <div>
                                                <div className="text-lg font-bold text-slate-800 dark:text-white">{news.openRate}</div>
                                                <div className="text-[10px] uppercase font-bold text-slate-400 flex items-center"><Eye size={10} className="mr-1" /> Open Rate</div>
                                            </div>
                                            <div>
                                                <div className="text-lg font-bold text-slate-800 dark:text-white">{news.clickRate}</div>
            ...
            <Modal isOpen={!!selectedNewsletter} onClose={() => setSelectedNewsletter(null)} title="" maxWidth="max-w-6xl">
                {selectedNewsletter && (
                    <div className="flex flex-col h-[85vh] bg-slate-50 dark:bg-slate-900">
                        ...
                        <div className="px-4 py-5 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex flex-col justify-center shrink-0">
                            ...
                            <h2 className="text-lg font-extrabold text-slate-800 dark:text-white tracking-tight line-clamp-1">
                        ...
                        <div className="flex-1 w-full bg-slate-200 dark:bg-slate-950/50 overflow-hidden flex flex-col">
                            ...
                            <div className="bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-3 flex items-center justify-center shrink-0">
                                <div className="w-full max-w-4xl flex items-center justify-between">
                                    ...
                                    <div className="bg-white dark:bg-slate-800 px-3 py-1 rounded-md text-xs text-slate-400 font-mono flex items-center shadow-sm border border-slate-200 dark:border-slate-700">
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 lg:p-5 flex justify-center">
                                <div className="bg-white dark:bg-slate-800 w-full max-w-4xl shadow-2xl rounded-xl overflow-hidden border border-slate-200/60 dark:border-slate-700 min-h-fit h-fit">
                                    ...
                                    {/* Email Body */}
                                    <div className="px-4 py-5 sm:px-16 sm:py-14">
                                        <div className="mb-5 border-b border-slate-100 dark:border-slate-700 pb-4 text-center">
                                            <h1 className="text-xl sm:text-xl font-serif font-bold text-slate-900 dark:text-white mb-3 leading-tight">
                                                {selectedNewsletter.subject}
                                            </h1>
                                        ...
                                        {/* Footer */}
                                        <div className="mt-16 pt-5 border-t border-slate-100 dark:border-slate-700 text-center">
