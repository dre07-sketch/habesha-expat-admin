                            ...
                            {[
                                {
                                    ...
                                    value: dashboardData.business.total_businesses?.toLocaleString() || 0,
                                    change: '+32',
                                    gradient: 'from-blue-500 to-cyan-500'
                                },
                                {
                                    icon: Star,
                                    label: 'Reviews',
                                    value: dashboardData.business.reviews?.total_reviews?.toLocaleString() || 0,
                                    change: '+87',
                                    gradient: 'from-amber-500 to-orange-500'
                                },
                                {
                                ...
                                <div
                                    key={i}
                                    className="relative bg-slate-900 dark:bg-slate-950 p-5 rounded-2xl border border-slate-800 dark:border-slate-700 text-white overflow-hidden group hover:shadow-2xl transition-all duration-300"
                                >
                                    ...
                                    <div className="relative z-10">
                                        <div className={`p-2 w-fit rounded-xl bg-gradient-to-br ${card.gradient} opacity-20 mb-4`}>
                            ...
                            {dashboardData.business.categories?.map((cat: any, i: number) => {
                                ...
                                return (
                                    <div key={i} className="group">
                                        <div className="flex justify-between items-center mb-3">
                                            <div className="flex items-center">
                                                <div className="p-2 rounded-xl bg-gradient-to-br from-slate-700 to-slate-600 mr-3 shadow-md">
                                                    <Briefcase size={14} className="text-white" />
                                                </div>
                                                <div className="text-sm font-bold text-slate-900 dark:text-white">{cat.category}</div>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <span className="font-black text-slate-900 dark:text-white text-lg">{cat.count}</span>
                                                <span className="text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-lg border border-emerald-500/20">+12%</span>
                                            </div>
                                        </div>
                                        <div className="relative">
                                            <div className="h-2 w-full bg-slate-100 dark:bg-slate-700/50 rounded-full overflow-hidden">
                                                <div
                                                    className="h-full bg-gradient-to-r from-slate-800 to-slate-600 dark:from-blue-500 dark:to-cyan-500 rounded-full transform origin-left transition-all duration-1000"
                                                    style={{ width: `${percentage}%` }}
                                                ></div>
                                            </div>
                                            <div className="text-[10px] text-slate-400 text-right mt-1.5 font-semibold">{percentage}%</div>
                                        </div>
                                    </div>
                                );
                            })}
