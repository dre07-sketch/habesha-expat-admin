...
const NewsletterForm: React.FC<NewsletterFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const textAreaClass = "w-full pl-5 pr-4 py-3 border border-blue-500/30 dark:border-blue-500/40 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white transition-all placeholder:text-slate-400 resize-none font-mono text-sm";
      ...
      {isSubmitting && (
        <div className="absolute inset-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md flex flex-col items-center justify-center rounded-xl animate-in fade-in duration-300">
          <div className="relative mb-3">
            ...
            <Send className="absolute inset-0 m-auto text-blue-600 animate-pulse" size={16} />
          </div>
          <h3 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight mb-2">Sending Campaign...</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="pb-2 border-b border-slate-100 dark:border-slate-700">
          <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center">
            <Mail className="mr-2 text-blue-500" size={14} /> Campaign Details
          </h3>
